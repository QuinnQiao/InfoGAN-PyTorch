...
from models.dsprites_model import Generator, Discriminator, DHead, QHead
from dataloader import get_data
from utils import *
from config_dsprites import params
...
if(params['dataset'] == 'dSprites'):
    ...
    assert 0, 'Not implemented for dataset: {}.'.format(params['dataset'])
...
for epoch in range(params['num_epochs']):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        for j in range(params['num_dis_c']):
            dis_loss += criterionQ_dis(q_logits[:, j*3 : j*3 + 3], target[j])
    ...
    if((epoch+1) == 1 or (epoch+1) == params['num_epochs']/2):
        ...
        vutils.save_image(vutils.make_grid(gen_data, nrow=10, padding=2, normalize=True), 
                            "sample_dsprites/Epoch_%d_{}.jpg".format(params['dataset']) %(epoch+1))
    ...
    if (epoch+1) % params['save_epoch'] == 0:
        torch.save({
            ...
            'params' : params
            }, 'checkpoint_dsprites/model_epoch_%d_{}'.format(params['dataset']) %(epoch+1))
...
with torch.no_grad():
    gen_data = netG(fixed_noise).detach().cpu()
vutils.save_image(vutils.make_grid(gen_data, nrow=10, padding=2, normalize=True),
                    "sample_dsprites/Epoch_%d_{}.jpg".format(params['dataset']) %(params['num_epochs']))
...
torch.save({
    ...
    'optimG' : optimG.state_dict(),
    'params' : params
    }, 'checkpoint_dsprites/model_final_{}'.format(params['dataset']))


G_losses = np.asarray(G_losses)
np.savetxt('checkpoint_dsprites/g_loss.txt', G_losses)
D_losses = np.asarray(D_losses)
np.savetxt('checkpoint_dsprites/d_loss.txt', D_losses)
