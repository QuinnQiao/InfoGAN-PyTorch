...
from models.mnist_model_ada import Generator, Discriminator, DHead, QHead
...
for epoch in range(params['num_epochs']):
    ...
    if((epoch+1) == 1 or (epoch+1) == params['num_epochs']/2):
        ...
        vutils.save_image(vutils.make_grid(gen_data, nrow=10, padding=2, normalize=True), 
                            "sample_ada/Epoch_%d_{}.jpg".format(params['dataset']) %(epoch+1))
    ...
    if (epoch+1) % params['save_epoch'] == 0:
        torch.save({
            ...
            'params' : params
            }, 'checkpoint_ada/model_epoch_%d_{}'.format(params['dataset']) %(epoch+1))
...
with torch.no_grad():
    gen_data = netG(fixed_noise).detach().cpu()
vutils.save_image(vutils.make_grid(gen_data, nrow=10, padding=2, normalize=True),
                    "sample_ada/Epoch_%d_{}.jpg".format(params['dataset']) %(params['num_epochs']))
...
torch.save({
    ...
    'optimG' : optimG.state_dict(),
    'params' : params
    }, 'checkpoint_ada/model_final_{}'.format(params['dataset']))


G_losses = np.asarray(G_losses)
np.savetxt('checkpoint_ada/g_loss.txt', G_losses)
D_losses = np.asarray(D_losses)
np.savetxt('checkpoint_ada/d_loss.txt', D_losses)
